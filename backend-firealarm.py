...
def get_fire_record():
    ...
    for r in room:
        result.append(
            {'fire': sum(r['flame']) / len(r['flame']), 'temp': sum(r['temp']) / len(r['temp']),
             'ref_temp': 50, 'gas': r['gas'],
...
def line_notify(alarm: dict):
    ...
    if ref['line_token'] is not None and ref['notification']:  # user set line_token and notification is on
